...
//	atk_8266_get_wanip(ipbuf);//获取WAN IP
	...
	printf("start trans...\r\n");
	u3_printf("GET https://api.seniverse.com/v3/weather/now.json?key=x3owc7bndhbvi8oq&location=zhengzhou&language=zh-Hans&unit=c\n\n");	
...
//	Show_Str(30,65,200,12,p,12,0);				//显示IP地址和端口	
	...
	u3_printf("GET https://api.seniverse.com/v3/weather/daily.json?key=x3owc7bndhbvi8oq&location=zhengzhou&language=zh-Hans&unit=c&start=0&days=5\n\n");
	delay_ms(20);//延时20ms返回的是指令发送成功的状态
//	atk_8266_at_response(1);
	USART3_RX_STA=0;	//清零串口3数据
	delay_ms(1000);
